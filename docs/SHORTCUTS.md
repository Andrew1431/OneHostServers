...
| 6 | Idle agent signals stop fire-and-forget | bash `idle-check.sh` (MACHINE_AGENT.md) | **Snapshot-from-quiescent-disk RESOLVED:** `provider.stop` now does an ACPI soft-off (`instances.stop` → wait for `TERMINATED`) *before* snapshotting, so the disk is consistent when captured — the on-VM contract (systemd graceful stop) is in `MACHINE_AGENT.md`. **Idle signal→teardown now works end-to-end** (a VM publishes `{kind:stop,id}`, the worker runs `provider.stop`; the `onehost-game-vm` SA + `onehost-server-id` metadata are wired by Terraform/provider). Still open for the *idle* path: (a) the signal is fire-and-forget (a dropped Pub/Sub publish = a VM that bills forever → needs a reconcile-sweep backstop), (b) idle stops have no Discord `interactionToken` so they produce no notification (worker reply-edit is a silent no-op; a channel-webhook path fixes it), (c) no authz on the job body — any topic publisher can stop any `id`, so per-server authz is the hardening, and (d) the worker's stop branch doesn't yet pass `allowAlreadyStopped` for idempotent idle/manual races (`provider.stop` already supports it) |
