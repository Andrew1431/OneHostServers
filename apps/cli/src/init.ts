...
export async function runInit(): Promise<void> {
  ...
  if (existsSync(tfvarsPath)) {
    ...
    if (p.isCancel(overwrite) || !overwrite) {
      ...
      writeFileSync(tfvarsPath, renderTfvars({ project, region, zone, sshRange }));
...
export function renderTfvars(o: {
  project: string;
  region: string;
  zone: string;
  sshRange: string;
...
# Game ports are per-server — opened via \`pnpm cli create --port …\` / \`cli ports …\`.
