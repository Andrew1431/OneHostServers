...
packages/
  ...
  dns          the DNS seam (interface + DuckDNS adapter) — stable address, off-box; NOT part of ServerProvider
...
pnpm cli dns <id> <duckdns-host> | --clear      # set/remove the stable address (DuckDNS) on an existing server; opt-in, needs DUCKDNS_TOKEN to publish
pnpm cli status <id>
